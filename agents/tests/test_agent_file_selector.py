...
class TestAgentFileSelector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if len(test_file_paths) < 3:
            raise ValueError(
                f"Expected at least 3 test files in {test_files_dir}, found {len(test_file_paths)}"
            )
    ...
    def test_select_by_batch(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                with patch.object(
                    Agent, "_query_and_extract", return_value="\n".join(expected_files)
                ) as mock_query:
    ...
    def test_select_by_batch_invalid_file(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                # Create patch for _query_and_extract
                with patch.object(
                    Agent, "_query_and_extract", return_value="nonexistent_file.py"
                ) as mock_query:
    ...
    def test_select_by_individual(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                # Create patch for _query_and_extract
                with patch.object(
                    Agent, "_query_and_extract", side_effect=responses
                ) as mock_query:
                    # Create patch for AgentFileSelector's _get_files
                    with patch.object(
                        AgentFileSelector,
                        ...
                        # Call the method
                        result = self.agent._select_by_individual(
                            issue_text, files_text
                        )
    ...
    def test_select_by_individual_invalid_response(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                # Create patch for _query_and_extract
                with patch.object(
                    Agent, "_query_and_extract", return_value="Maybe"
                ) as mock_query:
                    # Create patch for _get_files
                    with patch.object(
                        Agent,
                        ...
                        self.assertIn(
                            "Invalid response from model", str(context.exception)
                        )
    ...
    def test_forward_batch_method(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                with patch.object(
                    Agent, "_extract_tag", side_effect=[issue_text, files_text]
                ) as mock_extract_tag:
                    # Create patch for AgentFileSelector's _get_files
                    with patch.object(
                        AgentFileSelector, "_get_files", return_value=files_dict
                    ) as mock_get_files:
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                # Call the method
                                selected_files, output_text = self.agent.forward(
                                    text=test_file, method="individual"
                                )

                                # Verify results
                                self.assertEqual(selected_files, expected_files)
                                self.assertEqual(output_text, "test output")
                                mock_select_by_individual.assert_called_once_with(
                                    issue_text, files_text
                                )
                                # Update assertion to match actual call
                                mock_format_output.assert_called_once_with(
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                with patch.object(
                    ...
                    with self.assertRaises(ValueError) as context:
                        self.agent.forward(text=test_file, method="invalid_method")
    ...
    def test_forward_missing_issue_tag(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                # Create patch for _extract_tag
                with patch.object(
                    Agent, "_extract_tag", side_effect=[None, files_text]
                ) as mock_extract_tag:
                    with self.assertRaises(ValueError) as context:
                        self.agent.forward(text=test_file, method="batch")

                    self.assertIn(
                        "Could not extract <issue> tag", str(context.exception)
                    )
    ...
    def test_forward_missing_code_tag(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                # Create patch for _extract_tag
                with patch.object(
                    Agent, "_extract_tag", side_effect=[issue_text, None]
                ) as mock_extract_tag:
                    with self.assertRaises(ValueError) as context:
                        self.agent.forward(text=test_file, method="batch")

                    self.assertIn(
                        "Could not extract <code> tag", str(context.exception)
                    )
    ...
    def test_forward_custom_issue(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                # Create patch for _extract_tag
                with patch.object(
                    Agent, "_extract_tag", return_value=files_text
                ) as mock_extract_tag:
                    # Create patch for AgentFileSelector's _get_files
                    with patch.object(
                        AgentFileSelector, "_get_files", return_value=files_dict
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                output = self.agent._format_output(
                    test_file, files_text, [], files_dict
                )
    ...
    def test_format_output_no_code_block(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.agent._format_output(
                input_without_code, "Some files text", ["file1.py"], files_dict
            )
    ...
    def test_strip_line_num_true(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                files_dict = agent._get_files(
                    files_text_with_line_nums, agent.strip_line_num
                )
            ...
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                for file_path, content in files_dict.items():
                    ...
                    if content_lines:
                        ...
                        lines_with_numbers = [
                            line for line in content_lines if re.match(r"^\d+ ", line)
                        ]
                        self.assertTrue(
                            len(lines_with_numbers) > 0,
    ...
    def test_select_by_batch_with_strip_line_num(self):
        ...
        for test_idx, test_file in enumerate(self.test_files):
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                files_dict_with_nums = self._parse_files_to_dict(
                    files_text_with_line_nums
                )
                ...
                for path, content in files_dict_with_nums.items():
                    stripped_content = "\n".join(
                        [re.sub(r"^\d+ ", "", line) for line in content.splitlines()]
                    )
                ...
                with patch.object(
                    AgentFileSelector, "_get_files", return_value=files_dict_stripped
                ) as mock_get_files:
                    ...
                    with patch.object(
                        ...
                        # Call the method
                        result = agent_strip._select_by_batch(
                            issue_text, files_text_with_line_nums
                        )
                ...
                with patch.object(
                    AgentFileSelector, "_get_files", return_value=files_dict_with_nums
                ) as mock_get_files:
                    # Mock _query_and_extract to return fixed result
                    with patch.object(
                        Agent,
                        ...
                        # Call the method
                        result = agent_no_strip._select_by_batch(
                            issue_text, files_text_with_line_nums
                        )
            ...
            with self.subTest(f"Testing with test file {test_idx + 1}"):
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        # Call the method
                        result = agent_strip._select_by_individual(
                            issue_text, files_text_with_line_nums
                        )
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        # Call the method
                        result = agent_no_strip._select_by_individual(
                            issue_text, files_text_with_line_nums
                        )
